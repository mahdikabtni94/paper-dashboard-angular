...
@Component({
  selector: 'app-create-electronic',
  templateUrl: './create-electronic.component.html',
  styleUrls: ['./create-electronic.component.scss']
})
export class CreateelectronicComponent implements OnInit {

  imagePreview: string;
  statusList = ['enabled', 'disabled'];
  genders = ['male', 'female'];

  constructor(public dialogref: MatDialogRef<CreateelectronicComponent>,
  ...
  onSaveElectronic() {
    if (this.staffService.form.valid) {
      if (!this.staffService.form.get('emp_id').value) {
        this.staffService.AddElectronic(
        ...
        this.staffService.form.reset();
        this.staffService.initializeFormGroup();
        this.notificationService.success(':: Electronic Added successfully');
        this.onClose();
      } else {
        this.staffService.UpdateElectronics(
        ...
        this.staffService.form.reset();
        this.staffService.initializeFormGroup();
        this.notificationService.success(':: Electronic Updated successfully');
        this.onClose();
      }
    }
